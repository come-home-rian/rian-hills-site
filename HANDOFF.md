...
- [x] ~~Site was not mobile-friendly~~ — fixed 2026-08-28. The design-canvas export had zero `@media` queries: 12 fixed multi-column grid layouts that never stacked, headline text up to 92px fixed (unreadable on a phone), fixed 40px side gutters (21% of a 375px screen), and hero/feature blocks fixed at 520–660px tall. Fixed by rewriting the values themselves to be natively responsive rather than overriding them externally — `grid-template-columns` uses `repeat(auto-fit, minmax(...))` so grids stack on their own, headline sizes use `clamp(min, vw, max)`, hero/feature heights use `clamp()` scaled to viewport width, and side padding scales via `clamp(20px, 5vw, 40px)`. Verified at 1440px desktop (pixel-identical to the original: 92px hero text, 660px hero, 4-column grid, 40px gutters all unchanged) and at ~375–566px mobile widths (headline drops to 38px, hero to 360px, grids collapse to a single column, gutters to 20px) with no console errors. Note: this approach deliberately avoided CSS attribute-selector overrides (`[style*="..."]` + `!important`) after discovering the framework re-serializes inline styles through the DOM — hex colors become `rgb()`, `0` becomes `0px`, quotes get stripped — which silently breaks any override matching literal source-text substrings. If more mobile polish is needed later, keep editing actual values (clamp/minmax), not attribute-selector overrides.
