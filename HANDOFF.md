...
- [x] ~~Porcelain jars around the site weren't interactive (except the footer one)~~ — done 2026-08-29. Every jar now sends a bee flying out on click and then navigates home: the 4 per-section jar motifs (Interiors/Staging/Shop/Rian) got a new click handler + a small bee that pops out and fades before routing to `/`; the existing footer jar (with its own bee-to-hive animation) now also routes home once that sequence finishes, instead of just resetting in place. for `moriah0812@gmail.com` on Netlify — currently flagged "No 2FA" on the team members list. Worth doing now that there's a payment method on file for a real client-facing account.
