...
- [x] `/loves` ("Loved + Supported", issue #36) added 2026-09-03 as an 8th route: entry in the routing script's `valid` array, `SEO_TITLES`/`SEO_DESCS` rows, a new `isLoves` view in the template, header/footer nav entries, and a `sitemap.xml` row. The page's whole SEO purpose is outbound links to local rescues/charities, so when Rian's list lands, each org gets a real `<a href target="_blank" rel="noopener">` (no `nofollow` — passing local link equity is the point). If the page is still placeholder-only at launch, pull its `sitemap.xml` row until it has real content.
